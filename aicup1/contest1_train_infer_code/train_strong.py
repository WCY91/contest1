...
# model 2
    cfg = {
    "a_min": -100,        # 擴大低灰區（讓心肌邊界更明顯）
    "a_max": 400,         # 稍降上限，壓縮血池亮區
    "spacing": (0.7, 0.7, 0.7),  # 更等距、更細緻
    "roi_size": (128, 128, 96),  # 保持不變以防 OOM
    "batch_size": 1,
    "val_batch_size": 1,
    "num_classes": 4,
    "feature_size": 48,
    ...
    # ---------------- LOSS / OPT ----------------
    loss_fn = DiceCELoss(to_onehot_y=True, softmax=True, lambda_dice=0.7, lambda_ce=0.3)
    ...
    def lr_lambda(epoch): return min(1.0, float(epoch + 1) / 5)
    warmup_scheduler = LambdaLR(optimizer, lr_lambda=lr_lambda)
    cosine_scheduler = CosineAnnealingLR(optimizer, T_max=80, eta_min=1e-6)
    ...
    for epoch in range(cfg["max_epochs"]):
        ...
        if epoch < 12:
            ...
            lam_focal = 0.2 + 0.3 * min(1.0, max(0, epoch - 12) / 60)
            loss_fn = DiceFocalLoss(
                to_onehot_y=True, softmax=True,
                lambda_dice=0.7, lambda_focal=lam_focal,
                gamma=1.5,
            )
        ...
        if (epoch + 1) % cfg["val_every"] == 0:
            ...
            if score > best_score:
                best_score = score
                no_improve = 0
                torch.save(model.state_dict(), os.path.join(run_dir, "best_model2.pth"))
